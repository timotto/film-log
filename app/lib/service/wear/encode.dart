...
import 'package:film_log/model/film_stock.dart' as m;
...
w.FilmStock encodeFilmStock(
  m.FilmStock item, {
  required List<m.Camera> cameras,
}) =>
    w.FilmStock(
      id: item.id,
      label: '${item.manufacturer} ${item.product}',
      iso: item.iso,
      cameraIds: cameras
          .where((camera) => camera.filmstockFormat == item.format)
